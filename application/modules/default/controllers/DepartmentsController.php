            ...
            if (is_numeric($id) && $id > 0) {
                ...
                if (!empty($data)) {
                    ...
                    if (!empty($data['deptid'])) {
                        $parentDepartmentData = $deptModel->getSingleDepartmentData($data['deptid']);
                        if (!empty($parentDepartmentData)) {
                            $data['unitid'] = $parentDepartmentData['deptname'];
                        }
                    }
    ...
    public function editAction()
    {
        ...
        if (!empty($getorgData)) {
            ...
            if ($this->getRequest()->getPost()) {
                ...
                $unitdeptid = $this->_request->getParam('unitid');
                list($unitid, $deptid) = explode("_", $unitdeptid);
                if ($deptname != '' && ($unitid != '' || $deptid != '')) {
                    if (!preg_match('/^(?![0-9]{4})[a-zA-Z0-9.\- ?]+$/', $deptname)) {
                        $msgarray['deptname'] = "Please enter valid department name.";
                        $flag = 'false';
                    } else {
                        $checkExists = $deptModel->checkExistance($deptname, $unitid, $deptid, $id);
                        if ($checkExists != 0) {
