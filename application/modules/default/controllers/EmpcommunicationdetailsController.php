...
class Default_EmpcommunicationdetailsController extends Zend_Controller_Action
{
                                ...
                                if (!empty($empdata)) {
                                    ...
                                    if (sizeof($countrieslistArr) > 0) {
                                        ...
                                        foreach ($countrieslistArr as $countrieslistres) {
                                            ...
                                            $empcommdetailsform->work_country->addMultiOption($countrieslistres['id'], utf8_encode($countrieslistres['country_name']));
                                        }
                                    } else {
                                        $msgarray['perm_country'] = 'Countries are not configured yet.';
                                        $msgarray['current_country'] = 'Countries are not configured yet.';
                                        $msgarray['work_county'] = 'Countries are not configured yet.';
                                    }
                                    $data = $empcommdetailsModal->getsingleEmpCommDetailsData($id);
                                    if (!empty($data)) {
                                        ...
                                        $work_country = $data[0]['work_country'];
                                        if (isset($_POST['work_country'])) {
                                            $work_country = $_POST['work_country'];
                                        }
                                        $work_state = $data[0]['work_state'];
                                        if (isset($_POST['work_state'])) {
                                            $work_state = $_POST['work_state'];
                                        }
                                        $work_city = $data[0]['work_city'];
                                        if (isset($_POST['work_city'])) {
                                            $work_city = $_POST['work_city'];
                                        }
                                        if ($work_country != '') {
                                            $statePermlistArr = $statesmodel->getStatesList($work_country);
                                            if (sizeof($statePermlistArr) > 0) {
                                                $empcommdetailsform->work_state->addMultiOption('', 'Select State');
                                                foreach ($statePermlistArr as $statelistres) {
                                                    $empcommdetailsform->work_state->addMultiOption($statelistres['id'], $statelistres['state_name']);
                                                }
                                            }
                                        }
                                        if ($work_state != '') {
                                            $cityPermlistArr = $citiesmodel->getCitiesList($work_state);
                                            if (sizeof($cityPermlistArr) > 0) {
                                                $empcommdetailsform->work_city->addMultiOption('', 'Select City');
                                                foreach ($cityPermlistArr as $cityPermlistres) {
                                                    $empcommdetailsform->work_city->addMultiOption($cityPermlistres['id'], $cityPermlistres['city_name']);
                                                }
                                            }
                                        }
                                        ...
                                        if ($data[0]['work_country'] != '')
                                            $empcommdetailsform->setDefault('work_country', $work_country);
                                        if ($data[0]['work_state'] != '')
                                            $empcommdetailsform->setDefault('work_state', $work_state);
                                        if ($data[0]['work_city'] != '')
                                            $empcommdetailsform->setDefault('work_city', $work_city);
                                    }
    ...
    public function save($empcommdetailsform, $userid)
    {
        ...
        $work_country = $this->_request->getParam('work_country');
        $work_stateparam = $this->_request->getParam('work_state');
        $work_stateArr = explode("!@#", $this->_request->getParam('work_state'));
        $work_state = $work_stateArr[0];
        $work_cityparam = $this->_request->getParam('work_city');
        $work_cityArr = explode("!@#", $this->_request->getParam('work_city'));
        $work_city = $work_cityArr[0];
        ...
        if ($empcommdetailsform->isValid($this->_request->getPost())) {
            ...
            if (!empty($new_post_values)) {
                ...
                $work_streetaddress = $this->_request->getParam('work_streetaddress');
                $work_pincode = $this->_request->getParam('work_pincode');
                ...
                $data = array('user_id' => $user_id,
                    ...
                    'work_country' => ($work_country != '' ? $work_country : NULL),
                    'work_state' => ($work_state != '' ? $work_state : NULL),
                    'work_city' => ($work_city != '' ? $work_city : NULL),
