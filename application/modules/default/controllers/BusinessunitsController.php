...
class Default_BusinessunitsController extends Zend_Controller_Action
{
    ...
    public function editAction()
    {
        ...
        if (!empty($getorgData)) {
            ...
            $organisationHead = $employeeModel->getCurrentOrgHead();
            ...
            if (is_numeric($id) && $id > 0) {
                ...
                if (!empty($data)) {
                    ...
                    $managementUsersData = $deptModel->getDepartmenttHead($data['unithead']);
                    foreach ($managementUsersData as $mgmtdata) {
                        $businessunitsform->unithead->addMultiOption($mgmtdata['user_id'], $mgmtdata['userfullname']);
                    }
