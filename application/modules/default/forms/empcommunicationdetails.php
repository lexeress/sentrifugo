...
class Default_Form_empcommunicationdetails extends Zend_Form
{
    public function init()
    {
        ...
        $work_streetaddress = new Zend_Form_Element_Text('work_streetaddress');
        $work_streetaddress->setAttrib('maxLength', 100);
        $work_streetaddress->addFilter(new Zend_Filter_StringTrim());

        $work_country = new Zend_Form_Element_Select('work_country');
        $work_country->setAttrib('onchange', 'displayParticularState(this,"","work_state","")');
        $work_country->setRegisterInArrayValidator(false);
        $work_country->addMultiOption('', 'Select Country');

        $work_state = new Zend_Form_Element_Select('work_state');
        $work_state->setAttrib('onchange', 'displayParticularCity(this,"","work_city","")');
        $work_state->setRegisterInArrayValidator(false);
        $work_state->addMultiOption('', 'Select State');

        $work_city = new Zend_Form_Element_Select('work_city');
        $work_city->setRegisterInArrayValidator(false);
        $work_city->addMultiOption('', 'Select City');

        $work_pincode = new Zend_Form_Element_Text('work_pincode');
        $work_pincode->setAttrib('maxLength', 10);
        $work_pincode->addFilter(new Zend_Filter_StringTrim());
        $work_pincode->addValidators(array(array('StringLength', false,
            array('min' => 3,
                'max' => 10,
                'messages' => array(
                    Zend_Validate_StringLength::TOO_LONG =>
        ...
        $work_pincode->addValidator("regex", true, array(
            'pattern' => '/^(?!0{3})[0-9a-zA-Z]+$/',
            'messages' => array(
                'regexNotMatch' => 'Please enter valid postal code.'
            )
        ...
        $this->addElements(array($id, $userid, $personalemail, $perm_streetaddress, $perm_country, $perm_state, $perm_city, $perm_pincode, $current_streetaddress, $current_country, $current_state, $current_city, $current_pincode,
            $work_streetaddress, $work_country, $work_state, $work_city, $work_pincode, $address_flag, $emergency_number, $emergency_name, $emergency_email, $submit));
