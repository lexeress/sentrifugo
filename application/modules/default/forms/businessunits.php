...
class Default_Form_businessunits extends Zend_Form
{
    public function init()
    {
        ...
        $unithead = new Zend_Form_Element_Select('unithead');
        $unithead->setAttrib('class', 'selectoption');
        $unithead->setLabel('Unit Head');
        //$unithead->setRequired(true);
        //$unithead->addValidator('NotEmpty', false, array('messages' => 'Please select unit head.'));
        $unithead->addMultiOption('', 'Select Unit Head');
        $unithead->setRegisterInArrayValidator(false);
