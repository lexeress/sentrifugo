    ...
    jConfirm(messageAlert, "Delete " + msgdta, function (r) {
        ...
        if (r == true) {
            if (controllername == 'candidatedetails') {
                $.post(base_url + "/candidatedetails/chkcandidate", {
                    id: objid,
...
function getrolepopup(group_id, sort_name, sort_type, dialog_id) {
    ...
    $('#' + dialog_id).dialog({
        ...
        open: function () {
            ...
            $.post(base_url + "/reports/getrolepopup/format/html", {
                group_id: group_id,
                sort_name: sort_name,
                sort_type: sort_type
            }, function (data) {
...
function emprolesgroup_popup(group_id, role_id, page_no, sort_name, sort_type, dialog_id, per_page) {
    ...
    $('#' + dialog_id).dialog({
        ...
        open: function () {
            ...
            $('.ui-dialog').addClass('ui-btn-overwrite');
            $.blockUI({width: '50px', message: $("#spinner").html()});
            $.post(base_url + "/reports/emprolesgrouppopup/format/html", {
                per_page: per_page,
                group_id: group_id,
                role_id: role_id,
                page_no: page_no,
...
function getpositions_req(dept_id, bunit_id, position_id, job_id) {
    ...
    if (job_val != '') {
        $.post(base_url + "/default/requisition/getpositions", {
            bunitid: bunit_val,
            dept_id: dept_val,
            job_id: job_val
        }, function (data) {
...
function chk_future_date(obj, msg) {
    ...
    var to_val = '';
    if (from_val != '') {
        $.post(base_url + "/index/fromdatetodate", {
            from_val: from_val,
            to_val: to_val,
            con: 'future'
        }, function (data) {
...
function medicalclaimDates_validation(from_date_id, to_date_id, obj, datefield_id, conText) {
    ...
    if (from_val != '' && to_val != '') {
        $.post(base_url + "/index/medicalclaimdates", {
            from_val: from_val,
            to_val: to_val,
            new_to_val: new_to_val,
            con: conText,
            claimtype: txt
        }, function (data) {
            if (data.result == 'no') {
...
function makeActiveInactive_1(status, emp_id) {
    if (status == 'other') {
        ...
        jConfirm("Are you sure you want to " + mstatus + '?', 'Confirmation', function (r) {
            if (r == true) {
                $.post(base_url + "/default/employee/makeactiveinactive", {
                    emp_id: emp_id,
                    status: status
                }, function (data) {
                    if (data.result == 'yes') {
...
function validateorgandunitstartdate(ele, con) {
    ...
    if (ele.selectedIndex > 0) {
        var bunitdeptid = (ele[ele.selectedIndex].value).split('_');
        bunitid = bunitdeptid[0];
        deptid = bunitdeptid[1];
    }
